...
{
	template<typename J>
	friend class Graph; //graph can access me just fine
private:
...
public:
	...
	T* Geti(TreeNode<T>*, int, int&); //return the ith element in the tree using PreOrder i=0 corresponds to the root
	void ConverttoArray(TreeNode<T>*, int*, int& = 0);
};
#endif // !RedBlueTree_H
